<x-operator-layout>
    <x-slot name="title">
      Detail - Checkout {{ $checkout->item->name }}
    </x-slot>  
    <x-slot name="style">
      <style>
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="relative w-1/2 m-8">
                        ...
                        <li class="mb-10">
                          <div class="flex items-center mb-1">
                            <div class="bg-red-300 rounded-full h-8 w-8"></div>
                            <div class="flex-1 ml-4 font-medium">{{ $checkout->receipt_at->format('d F Y | ') }} {{ $checkout->receipt_at->format(' H:i:s') }} - <i class="fas fa-truck text-blue-400"></i> <span class="text-xs text-gray-400">{{ $checkout->operator->name }} menginput resi ({{ $checkout->courier }})</span></div>
                          </div>
                        </li> 
                        @endif
